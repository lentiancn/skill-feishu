...
| API 路径 | `https://open.feishu.cn/open-apis/contact/v3/departments/:department_id` |
|----------|---------------------------------------------------------------------------|
| 请求方法 | GET |
| 调用频率限制 | 特殊频控 |
| 支持的应用类型 | Custom App、Store App |
| 文档地址 | [飞书开放平台](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/get) |
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| `department_id` | string | 是 | 部门 ID，ID 类型需要与查询参数 `department_id_type` 的取值保持一致。<br>说明：<br>- 调用创建部门接口后，可从返回结果中获取到部门 ID 信息<br>- 部门 API 提供了多种获取-other 部门 ID 的方式，如获取子部门列表、获取父部门信息、搜索部门<br>最大长度：`64` 字符<br>正则校验：`^[a-zA-Z0-9][a-zA-Z0-9_\-@.]{0,63}$` |
...
```bash
curl -X GET 'https://open.feishu.cn/open-apis/contact/v3/departments/D096?user_id_type=open_id&department_id_type=open_department_id' \
...
def main():
    url = "https://open.feishu.cn/open-apis/contact/v3/departments/D096"
    headers = {
...
| 400 | 40015 | department not exist |
