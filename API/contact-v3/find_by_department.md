...
```bash
curl -X GET 'https://open.feishu.cn/open-apis/contact/v3/users/find_by_department?department_id=od-xxxxxxxxxxxxx&page_size=10' \
...
| 字段 | 类型 | 描述 |
|------|------|------|
| `union_id` | string | 用户的 union_id，是应用开发商发布的不同应用中同一用户的标识<br>**权限要求**：获取用户基本信息 (contact:user.base:readonly) |
