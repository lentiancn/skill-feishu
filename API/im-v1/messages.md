...
> **注意事项**：
> 1. **应用身份**发消息：需申请下面三个权限之一：**获取与发送单聊、群组消息**（im:message）、**以应用的身份发消息**（im:message:send_as_bot）、**发送消息V2【历史版本】**（im:message:send）
> 2. **用户身份**发消息：需同时申请以下两个权限：**获取与发送单聊、群组消息**（im:message）、**以用户身份发送消息**（im:message.send_as_user）
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| `receive_id_type` | string | 是 | 消息接收者 ID 类型<br>可选值：<br>- `open_id`：标识一个用户在某个应用中的身份<br>- `union_id`：标识一个用户在某个应用开发商下的身份<br>- `user_id`：标识一个用户在某个租户内的身份<br>- `email`：以用户的真实邮箱来标识用户<br>- `chat_id`：以群 ID 来标识群聊<br>**权限要求**：当值为 `user_id` 时，需要获取用户 user ID (contact:user.employee_id:readonly) |

### 请求体

| 名称 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `receive_id` | string | 是 | 消息接收者的 ID，ID 类型与查询参数 `receive_id_type` 的取值一致。<br>注意事项：<br>- 给用户发送消息时，用户需要在机器人的可用范围内<br>- 给群组发送消息时，机器人需要在该群组中，且在群组内拥有发言权限<br>- 如果消息接收者为用户，推荐使用用户的 `open_id` |
| `msg_type` | string | 是 | 消息类型<br>可选值：<br>- `text`：文本<br>- `post`：富文本<br>- `image`：图片<br>- `file`：文件<br>- `audio`：语音<br>- `media`：视频<br>- `sticker`：表情包<br>- `interactive`：卡片<br>- `share_chat`：分享群名片<br>- `share_user`：分享个人名片<br>- `system`：系统消息 |
| `content` | string | 是 | 消息内容，JSON 结构序列化后的字符串。该参数的取值与 `msg_type` 对应 |
| `uuid` | string | 否 | 自定义设置的唯一字符串序列，用于在发送消息时请求去重<br>数据校验规则：<br>- 最大长度：`50` 字符 |
...
| `msg_type` | string | 消息类型<br>可能值有：<br>- `text`：文本<br>- `post`：富文本<br>- `image`：图片<br>- `file`：文件<br>- `audio`：语音<br>- `media`：视频<br>- `sticker`：表情包<br>- `interactive`：卡片<br>- `share_chat`：分享群名片<br>- `share_user`：分享个人名片<br>- `system`：系统消息 |
...
| `id` | string | 发送者的 ID |
| `id_type` | string | 发送者的 ID 类型<br>可能值有：<br>- `open_id`：表示发送者为用户<br>- `app_id`：表示发送者为应用 |
| `sender_type` | string | 发送者类型<br>可能值有：<br>- `user`：用户<br>- `app`：应用<br>- `anonymous`：匿名（未生效）<br>- `unknown`：未知（未生效） |
| `tenant_key` | string | 租户唯一标识 |
| `body` | object | 通过 `body` 内的 `content` 参数，返回所发送的消息内容 |
| `content` | string | 消息内容，JSON 结构序列化后的字符串 |
| `mentions` | object[] | 发送的消息内，被 @ 的用户列表 |
...
#### data.body.mentions 数组中的对象

| 字段 | 类型 | 描述 |
|------|------|------|
| `key` | string | 被 @ 的用户序号 |
| `id` | string | 被 @ 的用户的 open_id |
| `id_type` | string | 被 @ 的用户的 ID 类型，目前仅支持 `open_id` |
| `name` | string | 被 @ 的用户姓名 |
| `tenant_key` | string | 租户唯一标识 |
...
| 11310 | Failed to create card content, ext=ErrCode: 11310; ErrMsg: %v |
...
| 链接类型 | 链接 |
|----------|------|
| 如何解决 230001 错误 | [https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-resolve-error-230001](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-resolve-error-230001) |
| 机器人使用指南 | [https://open.feishu.cn/document/ukTMukTMukTM/uATM04CMxQjLwEDN](https://open.feishu.cn/document/ukTMukTMukTM/uATM04CMxQjLwEDN) |
...
| 发送消息内容结构 | [https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/im-v1/message/create_json](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/im-v1/message/create_json) |
| 上传图片 | [https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/image/create](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/image/create) |
| 上传文件 | [https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/file/create](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/file/create) |
| 获取群信息 | [https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat/get](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat/get) |
