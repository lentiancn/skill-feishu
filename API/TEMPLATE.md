...
"相关链接"是接口文档本身应该有的一个章节，用于提供飞书官方文档的引用链接。对于模板来说，"相关链接"这个章节名称不能写死，而是根据接口实际情况自动生成。

附录下面可以包含多个二级分类章节（如"相关链接"），模板在生成接口文档时会自动根据接口的内容情况进行分类和组织。除了"相关链接"之外，如果接口有其他类型的补充信息，也可以在附录下创建新的二级分类章节。
