...
"相关链接"章节是根据具体接口文档实际情况自动生成的，**不是所有接口都必须有**。例如本模板参考的 find_by_department.md 接口包含"相关链接"章节，因此在模板示例中展示了其格式和内容。

模板在生成接口文档时，会根据接口实际情况自动判断是否需要添加"相关链接"章节，并按需进行分类组织。除了"相关链接"之外，如果接口有其他类型的补充信息，也可以在附录下创建新的二级分类章节。
